...
class BundlingMaterialSerialNumber implements Rule
{
    /**
    ...
    public function passes($attribute, $value)
    {
        ...
        $wheres[] = "(jid.jid_serial_number = '" . $value . "')";
        $wheres[] = '(jod.jod_deleted_on IS NULL)';
        $strWhere = ' WHERE ' . implode(' AND ', $wheres);
        $query = 'SELECT jid.jid_serial_number, SUM(jod.jod_quantity) as stock, (CASE WHEN jb.used IS NULL THEN 0 ELSE jb.used END) as total_used
                    ...
                    (SELECT jbm_serial_number, SUM(jbm_quantity) as used
                        ...
                        AND (jbm.jbm_id <> ' . $this->JbmId . ') AND (jbm.jbm_serial_number IS NOT NULL)
                        GROUP BY jbm_serial_number) as jb ON jid.jid_serial_number = jb.jbm_serial_number ' . $strWhere;
        $query .= ' GROUP BY jid.jid_serial_number, jb.used';
    ...
    {
        return trans('validation.invalidSerialNumber');
    }
}
