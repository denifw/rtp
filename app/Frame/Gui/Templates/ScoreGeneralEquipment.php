 ...
 * @copyright  2018 C-Book
 */
class ScoreGeneralEquipment extends AbstractTemplate
{
    /**
     * Attributes :
     * 1. title (OPTIONAL)
    ...
    protected function createTemplate(): string
    {
        ...
        if (empty($data) === false) {
            foreach ($data as $label => $value) {
                $dataText .= '<div style="float: left; width: 50%;padding-right: 10px;">';
                $dataText .= '<div style="float: left; width: 100%; font-size: 28px; font-weight: bold">' . $value . '</div>';
                $dataText .= '<div style="float: left; width: 100%; font-size: 15px; font-weight: normal">' . $label . '</div>';
                $dataText .= '</div>';
            }
        }
        $result .= '<div class="count" style="margin-top: 5px !important; text-align: center;">' . $dataText . '</div>';
        # param *label*
