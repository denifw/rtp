 ...
 * @copyright  2018 Spada
 */
class TotalJobComplete extends AbstractBaseWidgetDashboard
{
    /**w
    ...
    public function loadData(): float
    {
        ...
        if ($this->isValidParameter('jo_start_period') === true) {
            $orderDateFrom = "(jo_finish_on = '" . $this->getStringParameter('jo_start_period') . "')";
            if ($this->isValidParameter('jo_end_period') === true) {
                $orderDateFrom = "(jo_finish_on >= '" . $this->getStringParameter('jo_start_period') . "')";
            }
            $wheres[] = $orderDateFrom;
        }
        if ($this->isValidParameter('jo_end_period') === true) {
            $orderDateUntil = "(jo_finish_on = '" . $this->getStringParameter('jo_end_period') . "')";
            if ($this->isValidParameter('jo_start_period') === true) {
                $orderDateUntil = "(jo_finish_on <= '" . $this->getStringParameter('jo_end_period') . "')";
            }
            $wheres[] = $orderDateUntil;
        }
