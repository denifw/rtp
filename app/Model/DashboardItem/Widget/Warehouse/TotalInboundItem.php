 ...
 * @copyright  2018 Spada
 */
class TotalInboundItem extends AbstractBaseWidgetDashboard
{
    /**w
    ...
    public function loadData(): float
    {
        ...
        if ($this->isValidParameter('jo_start_period') === true) {
            $startFrom = "(ji.ji_end_load_on = '" . $this->getStringParameter('jo_start_period') . "')";
        ...
        if ($this->isValidParameter('jo_end_period') === true) {
            $startUntil = "(ji.ji_end_load_on = '" . $this->getStringParameter('jo_end_period') . "')";
            if ($this->isValidParameter('jo_start_period') === true) {
                $startUntil = "(ji.ji_end_load_on <= '" . $this->getStringParameter('jo_end_period') . "')";
            }
            $wheres[] = $startUntil;
        ...
        $query = 'SELECT sum(jid.jid_quantity) as total
