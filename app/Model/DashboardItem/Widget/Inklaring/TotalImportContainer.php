 ...
 * @copyright  2020 Spada
 */
class TotalImportContainer extends AbstractBaseWidgetDashboard
{
    /**w
    ...
    public function loadData(): float
    {
        ...
        $wheres[] = '(soc.soc_deleted_on IS NULL)';
        $wheres[] = '(soc.soc_ct_id IS NOT NULL)';
        ...
        $query = 'SELECT COUNT(soc.soc_id) as total
                  FROM job_order as jo INNER JOIN
                       ...
                       sales_order as so on jik.jik_so_id = so.so_id INNER JOIN
                        sales_order_container as soc ON so.so_id = soc.soc_so_id' . $strWhere;
