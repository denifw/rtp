 ...
 */
class SerialNumberHistory extends AbstractStatisticModel
{
    ...
    public function __construct(array $parameters)
    {
        ...
        parent::__construct(get_class($this), 'serialNumberHistory');
    ...
    public function loadSearchForm(): void
    {
        ...
        $this->StatisticForm->addField(Trans::getWord('serialNumber'), $this->Field->getText('jid_serial_number', $this->getStringParameter('jid_serial_number')), true);
        ...
        $this->StatisticForm->addField(Trans::getWord('warranty'), $this->Field->getYesNo('gd_warranty', $this->getStringParameter('gd_warranty')));
    ...
    public function loadValidationRole(): void
    {
        ...
        if ($this->isValidParameter('until_date') === true) {
            if ($this->isValidParameter('from_date') === true) {
                ...
                $this->Validation->checkDate('until_date');
            }
        }
    }
    ...
    {
        $temp = [];
        if ($this->isValidParameter('jid_serial_number') === true || $this->isValidParameter('from_date') === true || $this->isValidParameter('until_date') === true) {
    ...
    private function loadQuery(): string
    {
        ...
        if ($this->isValidParameter('gd_warranty') === true) {
            $joWheres[] = "(gd.gd_warranty = '" . $this->getStringParameter('gd_warranty') . "')";
        }
        if ($this->isValidParameter('jid_serial_number') === true) {
            $joWheres[] = SqlHelper::generateLikeCondition('jid.jid_serial_number', $this->getStringParameter('jid_serial_number'));
        }
        if ($this->isValidParameter('jid_packing_number') === true) {
            $joWheres[] = SqlHelper::generateLikeCondition('jid.jid_packing_number', $this->getStringParameter('jid_packing_number'));
        }
        ...
        return 'SELECT jo_type, jo_id, jo_srt_id, jo_number, so_number, customer_ref, aju_ref, bl_ref, packing_ref, sppb_ref,
                            container_number, seal_number, truck_number, wh_name, whs_name, load_on, shipper, jid_lot_number, jid_serial_number,
                            quantity, uom_code, gd_weight, gdt_id, damage_type, wh_id, gd_volume,
                            gd_id, gd_name, gd_sku, br_name, gdc_name, customer, gd_warranty, jid_packing_number, jid_expired_date
                FROM (' . $subQuery . ') as j
    ...
    private function loadInboundQuery(array $jiWheres): string
    {
        ...
        $query = "SELECT 'IN' as jo_type, jo.jo_id, jo.jo_srt_id, jo.jo_number, so.so_number, (CASE WHEN so.so_customer_ref IS NULL THEN jo.jo_customer_ref ELSE so.so_customer_ref END) as customer_ref,
                            ...
                            wh.wh_name, whs.whs_name, ji.ji_end_load_on as load_on, shp.rel_name as shipper, jid.jid_lot_number, jid.jid_serial_number,
                            ...
                            gd.gd_id, gd.gd_name, gd.gd_sku, br.br_name, gdc.gdc_name, rel.rel_short_name as customer, gd.gd_warranty, jid.jid_packing_number, jid.jid_expired_date
        ...
        $query .= ' GROUP BY jo.jo_id, jo.jo_srt_id, jo.jo_number, so.so_number, so.so_customer_ref, jo.jo_customer_ref,
                    ...
                    wh.wh_name, whs.whs_name, ji.ji_end_load_on, shp.rel_name, jid.jid_lot_number, jid.jid_serial_number,
                    uom.uom_code, jid.jid_weight, gdu.gdu_weight,
                    gdt.gdt_id, gdt.gdt_description, wh.wh_id, jid.jid_volume, gdu.gdu_volume,
                    gd.gd_id, gd.gd_name, gd.gd_sku, br.br_name, gdc.gdc_name, rel.rel_short_name, gd.gd_warranty, jid.jid_packing_number, jid.jid_expired_date';
    ...
    private function loadOutboundQuery(array $jobWheres): string
    {
        ...
        $query = "SELECT 'OUT' as jo_type, jo.jo_id, jo.jo_srt_id, jo.jo_number, so.so_number, (CASE WHEN so.so_customer_ref IS NULL THEN jo.jo_customer_ref ELSE so.so_customer_ref END) as customer_ref,
                            ...
                            wh.wh_name, whs.whs_name, job.job_end_store_on as load_on, con.rel_name as shipper, jid.jid_lot_number, jid.jid_serial_number,
        ...
        $query .= ' GROUP BY jo.jo_id, jo.jo_srt_id, jo.jo_number, so.so_number, so.so_customer_ref, jo.jo_customer_ref,
                        ...
                        wh.wh_name, whs.whs_name, job.job_end_store_on, con.rel_name, jid.jid_lot_number, jid.jid_serial_number,
                        uom.uom_code, jid.jid_weight, gdu.gdu_weight,
                        gdt.gdt_id, gdt.gdt_description, wh.wh_id, jid.jid_volume, gdu.gdu_volume,
                        gd.gd_id, gd.gd_name, gd.gd_sku, br.br_name, gdc.gdc_name, rel.rel_short_name, gd.gd_warranty, jid.jid_packing_number, jid.jid_expired_date';
    ...
    private function loadMovementQuery(array $jmWheres): string
    {
        ...
        $query = "SELECT 'MOV' as jo_type, jo.jo_id, jo.jo_srt_id, jo.jo_number, '' as so_number, jo.jo_aju_ref as customer_ref, jo.jo_aju_ref as aju_ref,
                            ...
                            wh.wh_name, whs.whs_name || '=>' || whsm.whs_name as whs_name , jm.jm_complete_on as load_on, '' as shipper, jid.jid_lot_number, jid.jid_serial_number,
    ...
    private function loadAdjustmentQuery(array $jaWheres): string
    {
        ...
        $query = "SELECT 'ADJ' as jo_type, jo.jo_id, jo.jo_srt_id, jo.jo_number, '' as so_number, jo.jo_aju_ref as customer_ref, jo.jo_aju_ref as aju_ref,
                        jo.jo_bl_ref as bl_ref, jo.jo_packing_ref as packing_ref, jo.jo_sppb_ref as sppb_ref,
                        '' as container_number, '' as seal_number, '' as truck_number,
                        wh.wh_name, whs.whs_name as whs_name , ja.ja_complete_on as load_on, '' as shipper, jid.jid_lot_number, jid.jid_serial_number,
    ...
    private function doPrepareData(array $data): void
    {
        ...
        foreach ($data as $row) {
            ...
            #waranty Preparation
            if ((int)$row['jo_srt_id'] === 2 && $row['gd_warranty'] === 'Y') {
                if (\array_key_exists($row['gd_sku'], $this->WarantyData) === false) {
                    $this->WarantyData[$row['gd_sku']] = (float)$row['quantity'];
                } else {
                    $this->WarantyData[$row['gd_sku']] += (float)$row['quantity'];
                }
            }
    ...
    protected function getTableView(string $htmlId): Table
    {
        ...
        $table->setHeaderRow([
            ...
            'gd_sku' => Trans::getWord('modelId'),
            'gd_warranty' => Trans::getWord('warranty'),
        ...
        $table->setColumnType('gd_warranty', 'yesno');
    ...
    public function doExportXls(): void
    {
        ...
        foreach ($this->Datas as $key => $portlet) {
            if (empty($portlet->Body) === false && ($portlet->Body[0] instanceof Table)) {
                ...
                if (empty($this->WarantyData) === false) {
                    $excel->addSheet('Out_Waranty', 'Out_Waranty');
                    $excel->setActiveSheet('Out_Waranty');
                    $sheetWaranty = $excel->getSheet('Out_Waranty', true);
                    $warantyTable = new ExcelTable($excel, $sheetWaranty);
                    $warantyTable->setTable($this->getWarrantyTable());
                    $warantyTable->writeTable();
                }
                $excel->setActiveSheet($sheetName);
            }
        }
     ...
     * @return Table
     */
    private function getWarrantyTable(): Table
    {
        $table = new Table('TblWarranty');
        $table->setHeaderRow([
            'sku' => Trans::getWord('modelId'),
            'qty' => Trans::getWord('quantity'),
        ]);
        $rows = [];
        foreach ($this->WarantyData as $key => $val) {
            $rows[] = [
                'sku' => $key,
                'qty' => $val,
            ];
