...
class JobSalesDao extends AbstractBaseDao
{
    ...
    {
        $wheres = [];
        $wheres[] = SqlHelper::generateStringCondition('jos_id', $referenceValue);
    ...
    public static function getByReferenceAndSystem(string $referenceValue, string $ssId): array
    {
        $wheres = [];
        $wheres[] = SqlHelper::generateStringCondition('jos_id', $referenceValue);
        $wheres[] = SqlHelper::generateStringCondition('jos_ss_id', $ssId);
        $data = self::loadData($wheres);
        if (count($data) === 1) {
            return $data[0];
        }
    ...
    public static function loadTotalData(array $wheres = []): int
    {
        ...
        $query = 'SELECT count(DISTINCT (jos_id)) AS total_rows
                        FROM job_sales' . $strWhere;
    ...
    public static function loadSingleSelectData($textColumn, array $wheres = [], array $orders = []): array
    {
        ...
        return parent::doPrepareSingleSelectData($data, $textColumn, 'jos_id');
    }
