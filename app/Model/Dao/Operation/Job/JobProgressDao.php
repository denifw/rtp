...
/**
 * Class to handle data access object for table job_progress.
 *
 * @package    app
 ...
 * @copyright  2021 Deni Firdaus Waruwu.
 */
class JobProgressDao extends AbstractBaseDao
{
    /**
    ...
    /**
     * Base dao constructor for job_progress.
     *
     */
    public function __construct()
    {
        parent::__construct('job_progress', 'jop', self::$Fields);
    }
    ...
    {
        $wheres = [];
        $wheres[] = SqlHelper::generateStringCondition('jop_id', $referenceValue);
    ...
    public static function getByReferenceAndSystem(string $referenceValue, string $ssId): array
    {
        $wheres = [];
        $wheres[] = SqlHelper::generateStringCondition('jop_id', $referenceValue);
        $wheres[] = SqlHelper::generateStringCondition('jop_ss_id', $ssId);
        $data = self::loadData($wheres);
        if (count($data) === 1) {
            return $data[0];
        }
    ...
    public static function loadTotalData(array $wheres = []): int
    {
        ...
        $query = 'SELECT count(DISTINCT (jop_id)) AS total_rows
                        FROM job_progress' . $strWhere;
    ...
    public static function loadSingleSelectData($textColumn, array $wheres = [], array $orders = []): array
    {
        ...
        return parent::doPrepareSingleSelectData($data, $textColumn, 'jop_id');
    }
