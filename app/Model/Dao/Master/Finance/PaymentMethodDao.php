...
class PaymentMethodDao extends AbstractBaseDao
{
    ...
    public static function getByReferenceAndSystem(string $referenceValue, string $ssId): array
    {
        ...
        $helper->addStringWhere('pm_id', $referenceValue);
        $helper->addStringWhere('pm_ss_id', $ssId);
    ...
    public static function loadData(SqlHelper $helper): array
    {
        if ($helper->hasOrderBy() === false) {
            $helper->addOrderByString('pm_name, pm_id');
        }
        $query = 'SELECT pm_id, pm_name, pm_active, pm_ss_id
                FROM payment_method ' . $helper;
    ...
    public static function loadTotalData(SqlHelper $helper): int
    {
        ...
        $query = 'SELECT count(DISTINCT (pm_id)) AS total_rows
                   FROM payment_method ' . $helper->getConditionForCountData();
