...
class JobTitleDao extends AbstractBaseDao
{
    ...
    {
        parent::__construct('job_title', 'jt', self::$Fields);
    }
    ...
    {
        $wheres = [];
        $wheres[] = SqlHelper::generateStringCondition('jt_id', $referenceValue);
    ...
    public static function getByReferenceAndSystem(string $referenceValue, string $ssId): array
    {
        $wheres = [];
        $wheres[] = SqlHelper::generateStringCondition('jt_id', $referenceValue);
        $wheres[] = SqlHelper::generateStringCondition('jt_ss_id', $ssId);
        $data = self::loadData($wheres);
        if (count($data) === 1) {
            return $data[0];
        }
    ...
    public static function loadData(array $wheres = [], array $orders = [], int $limit = 0, int $offset = 0): array
    {
        ...
        $query = 'SELECT jt.jt_id, jt.jt_ss_id, jt.jt_description, jt.jt_deleted_on, ud.us_name as jt_deleted_by,
                        uc.us_name as jt_created_by, jt.jt_created_on, jt.jt_deleted_reason
                        FROM job_title as jt
                            INNER JOIN users as uc ON uc.us_id = jt.jt_created_by
                            LEFT OUTER JOIN users as ud ON ud.us_id = jt.jt_deleted_by ' . $strWhere;
        if (empty($orders) === false) {
            $query .= ' ORDER BY ' . implode(', ', $orders);
        } else {
            $query .= ' ORDER BY jt_description, jt_id';
        }
        if ($limit > 0) {
    ...
    public static function loadTotalData(array $wheres = []): int
    {
        ...
        $query = 'SELECT count(DISTINCT (jt.jt_id)) AS total_rows
                        FROM job_title as jt
                            INNER JOIN users as uc ON uc.us_id = jt.jt_created_by
                            LEFT OUTER JOIN users as ud ON ud.us_id = jt.jt_deleted_by' . $strWhere;
    ...
    public static function loadSingleSelectData($textColumn, array $wheres = [], array $orders = []): array
    {
        ...
        return parent::doPrepareSingleSelectData($data, $textColumn, 'jt_id');
    }
