...
class PageCategoryDao extends AbstractBaseDao
{
    ...
    {
        $wheres = [];
        $wheres[] = SqlHelper::generateStringCondition('pc_id', $referenceValue);
    ...
    public static function loadData(array $wheres = [], array $orderBy = [], int $limit = 0, int $offset = 0): array
    {
        ...
        $query = 'SELECT pc_id, pc_code, pc_name, pc_route, pc_active
                        FROM page_category' . $strWhere;
        if (empty($orderBy) === false) {
            $query .= ' ORDER BY ' . implode(', ', $orderBy);
        } else {
            $query .= ' ORDER BY pc_name, pc_id';
        }
        if ($limit > 0) {
    ...
    public static function loadSingleSelectData($textColumn, array $wheres = [], array $orders = []): array
    {
        ...
        return parent::doPrepareSingleSelectData($data, $textColumn, 'pc_id');
    }
