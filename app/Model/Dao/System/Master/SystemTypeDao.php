...
class SystemTypeDao extends AbstractBaseDao
{
    ...
    {
        $wheres = [];
        $wheres[] = SqlHelper::generateStringCondition('sty_id', $referenceValue);
    ...
    public static function loadData(array $wheres = [], array $orders = [], int $limit = 0, int $offset = 0): array
    {
        ...
        $query = 'SELECT sty_id, sty_group, sty_code, sty_name, sty_active
                        FROM system_type' . $strWhere;
        if (empty($orders) === false) {
            $query .= ' ORDER BY ' . implode(', ', $orders);
        } else {
            $query .= ' ORDER BY sty_group, sty_name, sty_id';
        }
        if ($limit > 0) {
    ...
    public static function loadTotalData(array $wheres = []): int
    {
        ...
        $query = 'SELECT count(DISTINCT (sty_id)) AS total_rows
                        FROM system_type' . $strWhere;
    ...
    public static function loadSingleSelectData($textColumn, array $wheres = [], array $orders = []): array
    {
        ...
        return parent::doPrepareSingleSelectData($data, $textColumn, 'sty_id');
    }
