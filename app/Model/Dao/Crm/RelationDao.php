...
class RelationDao extends AbstractBaseDao
{
    ...
    public static function getByReference(string $relId): array
    {
        ...
        $helper->addStringWhere('rel.rel_id', $relId);
    ...
    public static function getByReferenceAndSystem(string $relId, string $ssId): array
    {
        $helper = new SqlHelper();
        $helper->addStringWhere('rel.rel_id', $relId);
        $helper->addStringWhere('rel.rel_ss_id', $ssId);
        $results = self::loadData($helper);
        if (count($results) === 1) {
            return $results[0];
        }
    ...
    public static function loadData(SqlHelper $helper): array
    {
        if ($helper->hasOrderBy() === false) {
            $helper->addOrderByString('rel.rel_name, rel.rel_id');
        }
        $query = 'SELECT rel.rel_id, rel.rel_ss_id, ss.ss_relation as rel_system,
                  ...
                  FROM   relation as rel
                         ...
                         LEFT OUTER JOIN contact_person as cp on rel.rel_cp_id = cp.cp_id ' . $helper;
    ...
    public static function loadTotalData(SqlHelper $helper): int
    {
        ...
        $query = 'SELECT count(DISTINCT (rel.rel_id)) AS total_rows
                  FROM relation as rel
                         INNER JOIN system_setting as ss on rel.rel_ss_id = ss.ss_id
                         LEFT OUTER JOIN office as o on rel.rel_of_id = o.of_id
                         LEFT OUTER JOIN contact_person as cp on rel.rel_cp_id = cp.cp_id' . $helper->getConditionForCountData();
