...
class JobBundling extends BaseJobOrder
{
    ...
    protected function doUpdate(): void
    {
        if ($this->getFormAction() === 'doActionStartPicking') {
        ...
        } elseif ($this->getFormAction() === 'doActionStartBundling') {
            # Create date time action
            $dateTime = $this->getStringParameter('jac_date') . ' ' . $this->getStringParameter('jac_time');
            # Update start store job.
            $jbColVal = [
            ...
            $query = 'SELECT jog.jog_id, gm.gm_quantity, gd.gd_sn
            ...
            $jidData = JobInboundDetailDao::getByReference($this->getIntParameter('jbm_jid_id'));
            $jbmColVal = [
                'jbm_jid_id' => $this->getIntParameter('jbm_jid_id'),
                'jbm_serial_number' => $jidData['jid_serial_number'],
                'jbm_gd_id' => $jidData['jid_gd_id'],
                'jbm_gdu_id' => $jidData['jid_gdu_id'],
                'jbm_packing_number' => $jidData['jid_packing_number'],
                'jbm_lot_number' => $jidData['jid_lot_number'],
                'jbm_gdt_id' => $jidData['jid_gdt_id'],
                'jbm_gdt_remark' => $jidData['jid_gdt_remark'],
                'jbm_gcd_id' => $jidData['jid_gcd_id'],
                'jbm_gcd_remark' => $jidData['jid_gcd_remark'],
                'jbm_expired_date' => $jidData['jid_expired_date'],
                'jbm_length' => $jidData['jid_length'],
                'jbm_width' => $jidData['jid_width'],
                'jbm_height' => $jidData['jid_height'],
                'jbm_volume' => $jidData['jid_volume'],
                'jbm_weight' => $jidData['jid_weight']
            ];
            $jbmDao = new JobBundlingMaterialDao();
        ...
        } elseif ($this->getFormAction() === 'doActionEndBundling') {
            # Create date time action
            $dateTime = $this->getStringParameter('jac_date') . ' ' . $this->getStringParameter('jac_time');
            ...
            if ($this->getStringParameter('jog_gd_sn') === 'Y') {
                $jbdData = JobBundlingDetailDao::getByJobBundling($this->getIntParameter('jb_id'));
                foreach ($jbdData AS $jbd) {
                    $jbmData = JobBundlingMaterialDao::getByJobBundlingDetail($jbd['jbd_id']);
                    $length = 0;
                    $height = 0;
                    $width = 0;
                    $volume = 0;
                    $weight = 0;
                    foreach ($jbmData AS $jbm) {
                        $length += $jbm['jbm_length'];
                        $height += $jbm['jbm_height'];
                        $width += $jbm['jbm_width'];
                        $volume += $jbm['jbm_volume'];
                        $weight += $jbm['jbm_weight'];
                    }
                    # Create Inbound Receive.
                    $jirColVal = [
                        'jir_ji_id' => $jiDao->getLastInsertId(),
                        'jir_jog_id' => $this->getIntParameter('jb_jog_id'),
                        'jir_quantity' => 1,
                        'jir_qty_damage' => 0,
                        'jir_stored' => 'Y',
                        'jir_serial_number' => $jbd['jbd_serial_number'],
                        'jir_lot_number' => $jbd['jbd_lot_number'],
                        'jir_length' => $length,
                        'jir_width' => $width,
                        'jir_height' => $height,
                        'jir_volume' => $volume,
                ...
                $jirColVal = [
                    'jir_ji_id' => $jiDao->getLastInsertId(),
                    'jir_jog_id' => $this->getIntParameter('jb_jog_id'),
                    'jir_quantity' => $this->getFloatParameter('jog_quantity'),
                    'jir_qty_damage' => 0,
                    'jir_stored' => 'Y'
                ];
                $jirDao = new JobInboundReceiveDao();
    ...
    public function loadValidationRole(): void
    {
        if ($this->getFormAction() === 'doActionStartPicking') {
            ...
            if ($this->getStringParameter('jog_gd_sn', 'N') === 'Y') {
                ...
                $this->Validation->checkSnBundlingDetail('jbd_serial_number_add', $this->getIntParameter('jog_gd_id'), 0);
            }
        } elseif ($this->getFormAction() === 'doDeleteBundle') {
            $this->Validation->checkRequire('jbd_id_del');
        } elseif ($this->getFormAction() === 'doBundleMaterial') {
            $this->Validation->checkRequire('jbm_id');
            $this->Validation->checkRequire('jbm_jid_id');
            $this->Validation->checkRequire('jbm_serial_number');
        } elseif ($this->getFormAction() === 'doCompleteBundle') {
            $this->Validation->checkRequire('jbd_id');
            if ($this->getStringParameter('jog_gd_sn', 'N') === 'Y') {
                $this->Validation->checkRequire('jbd_serial_number');
                $this->Validation->checkSnBundlingDetail('jbd_serial_number', $this->getIntParameter('jog_gd_id'), $this->getIntParameter('jbd_id'));
            }
            ...
            if ($this->getStringParameter('jid_gd_packing', 'N') === 'Y') {
                $this->Validation->checkRequire('jid_packing_number');
            }
            if ($this->getStringParameter('jid_gd_expired', 'N') === 'Y') {
                $this->Validation->checkRequire('jid_expired_date');
            }
        } elseif ($this->getFormAction() === 'doDeletePutAway') {
    ...
    protected function getPickingStorageModal(): Modal
    {
        ...
        $jogField->addParameter('jog_ignore_id', $this->getIntParameter('jb_jog_id'));
    ...
    protected function getCreateBundleModal(): Modal
    {
        ...
        $fieldSet->addField(Trans::getWord('serialNumber'), $this->Field->getText('jbd_serial_number_add', $this->getStringParameter('jbd_serial_number_add')));
        if ($this->getStringParameter('jog_gd_sn', 'N') === 'Y') {
    ...
    private function getBundlingProgressFieldSet(): Portlet
    {
        ...
        $fieldSet->addField(Trans::getWord('serialNumber'), $this->Field->getText('jbd_serial_number', $this->getStringParameter('jbd_serial_number')));
        $fieldSet->addHiddenField($this->Field->getHidden('jbd_id', $this->getIntParameter('jbd_id')));
        if ($this->getStringParameter('jog_gd_sn', 'N') === 'Y') {
            $fieldSet->setRequiredFields(['jbd_serial_number']);
        }
        # Create a portlet box.
    ...
    protected function getBundleMaterialFieldSet(bool $showModal): FieldSet
    {
        ...
        $snField = $this->Field->getSingleSelect('jobOutboundDetail', 'jbm_serial_number', $this->getParameterForModal('jbm_serial_number', $showModal), 'loadSerialNumberForBundling');
        $snField->setHiddenField('jbm_jid_id', $this->getParameterForModal('jbm_jid_id', $showModal));
        $snField->setEnableDetailButton(false);
        $snField->setEnableNewButton(false);
        $snField->addParameterById('jog_id', 'jbm_jog_id', Trans::getWord('goods'));
        $snField->addParameter('job_id', $this->getIntParameter('jb_outbound_id'));
        $snField->addParameter('jb_id', $this->getIntParameter('jb_id'));
        $snField->addParameterById('jbm_id', 'jbm_id', Trans::getWord('bundling'));
        # Add field into field set.
        $fieldSet->addField(Trans::getWord('sku'), $skuField);
        $fieldSet->addField(Trans::getWord('goods'), $goodsField);
        $fieldSet->addField(Trans::getWord('serialNumber'), $snField, true);
    ...
    protected function getPutAwayFieldSet(string $modalId, bool $showModal): FieldSet
    {
        ...
        $jirField->setAutoCompleteFields([
            ...
            'jid_gd_sn' => 'jir_gd_sn',
            'jid_gd_packing' => 'jir_gd_packing',
            'jid_gd_expired' => 'jir_gd_expired',
            'jid_length' => 'jir_length',
            'jid_width' => 'jir_width',
            'jid_height' => 'jir_height',
            'jid_weight' => 'jir_weight',
            'jid_volume' => 'jir_volume',
        ]);
        ...
        $expireField = $this->Field->getCalendar('jid_expired_date', $this->getParameterForModal('jid_expired_date', $showModal));
        # Add field into field set.
        ...
        $fieldSet->addField(Trans::getWord('expiredDate'), $expireField);
        ...
        $fieldSet->addHiddenField($this->Field->getHidden('jid_gd_packing', $this->getParameterForModal('jid_gd_packing', $showModal)));
        $fieldSet->addHiddenField($this->Field->getHidden('jid_gd_expired', $this->getParameterForModal('jid_gd_expired', $showModal)));
