...
class JobOrder extends BaseJobOrder
{
    ...
    public function __construct(array $parameters)
    {
        ...
        parent::__construct(get_class($this), 'jobOrder', $parameters);
    }
    ...
    public function loadSearchForm(): void
    {
        ...
        if ($this->PageSetting->checkPageRight('ThirdPartyAccess') === false) {
            ...
            $this->ListingForm->addHiddenField($this->Field->getHidden('jo_rel_id', $this->User->getRelId()));
        }
    ...
    private function doPrepareData(array $data): array
    {
        ...
        foreach ($data as $row) {
            ...
            $row['jo_action'] = $btn;
            if ($this->PageSetting->checkPageRight('AllowUpdate') === false) {
                $btnUpdate = new HyperLink('BtnEdit' . $row['jo_id'], '', $jobDao->getJobUrl('detail', (int)$row['jo_srt_id'], $row['jo_id']), true);
